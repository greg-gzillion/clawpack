"""Shared memory module for agentforlaw"""
import sqlite3
...
class ClawMemory:
    def __init__(self, agent_name: str = "agentforlaw"):
    ...
    def _init_tables(self):
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS memories (
            ...
            agent TEXT, key TEXT, value TEXT, timestamp TEXT, tags TEXT)''')
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS agent_registry (
            agent_id TEXT PRIMARY KEY, name TEXT, repo TEXT, capabilities TEXT, last_seen TEXT)''')
        self.conn.commit()
    ...
    def recall(self, key: str):
        self.cursor.execute("SELECT agent, key, value, tags FROM memories WHERE key LIKE ? ORDER BY timestamp DESC LIMIT 10",
                           (f"%{key}%",))
